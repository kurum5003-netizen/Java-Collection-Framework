...
public class GameEngine {
    ...
    private void handleCommand(String command) {
        ...
        switch (parts[0].toLowerCase()) {
            ...
            case "go":
                if (parts.length < 2) System.out.println("Nereye gideceksin?");
                else player.move(parts[1]); break;
            case "take":
                if (parts.length < 2) System.out.println("Ne alacaksın?");
                else player.take(parts[1]); break;
            case "use":
            ...
            case "talk":
                if (parts.length < 2) System.out.println("Kiminle konuşacaksın?");
                else {
                    ...
                    if (!found) System.out.println("Böyle bir NPC yok veya konuşulamaz.");
                }
                break;
            case "attack":
                if (parts.length < 2) System.out.println("Kimi atacaksın?");
                else {
                    String npcName = parts[1];
                    boolean found = false;
                    for (NPC n : npcs) {
